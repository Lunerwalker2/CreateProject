...
mouse.set_pos(center)

mouseRel = mouse.get_pos()

bg = pg.image.load("Images/field-skystone-dark-fix.jpg")
...
class Robot:
    ...
    def draw(self, surf):
        pg.draw.circle(surf, ROBOT_COLOR, (int(self.x * PIX_PER_INCH), int(self.y * PIX_PER_INCH)),
                       int(self.width / 2 * PIX_PER_INCH), 2)
        pg.draw.line(surf, HEADING_COLOR, (int(self.x * PIX_PER_INCH), int(self.y * PIX_PER_INCH)), (
        int((self.x + (math.cos(self.a) * 14)) * PIX_PER_INCH), int((self.y + (math.sin(self.a) * 14)) * PIX_PER_INCH)),
                     2)
        for path in self.paths:
            pg.draw.line(surf, LINE_COLOR, (int(path[0][0] * PIX_PER_INCH), int(path[0][1] * PIX_PER_INCH)),
                         (int(path[1][0] * PIX_PER_INCH), int(path[1][1] * PIX_PER_INCH)), 2)
